...
@section('description', 'Lihat pengumuman terbaru dan penting dari kami.')
...
<div class="bg-white p-6 rounded-lg border border-gray-200">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 pb-4 border-b border-gray-200">
        <h1 class="text-3xl font-bold text-[#03346E] mb-2 md:mb-0">
            <i class="fas fa-bullhorn text-[#03346E] mr-3"></i> Pengumuman
        </h1>
    </div>

    {{-- Hapus bagian PHP dummy data ini, karena data akan datang dari controller --}}
    {{--
    <?php
    ...
    <div class="prose max-w-none mb-8">
        <h2 class="text-2xl font-bold text-[#03346E]">Pengumuman Pengajuan Persetujuan Lingkungan</h2>
        <p class="text-gray-600">Berikan Saran, Tanggapan dan Pendapat Anda!</p>
    </div>

    @if (!empty($pengumumanItems) && count($pengumumanItems) > 0)
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach ($pengumumanItems as $item)
                <div class="bg-white border border-gray-200 rounded-xl p-4 flex flex-col transition-all duration-200 ease-in-out">
                    ...
                    <p class="text-sm text-gray-600 mb-1">
                        <i class="fas fa-industry mr-2"></i> Bidang Usaha: {{ $item['bidang_usaha'] ?? 'N/A' }}
                    </p>
    ...
    @else
        <p class="text-gray-600 text-center py-8">Tidak ada pengumuman yang tersedia saat ini.</p>
    @endif
</div>
@endsection